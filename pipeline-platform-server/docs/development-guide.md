...
> 2026-08-16 整理，作为下一步开发/运维的清单。2026-08-24 更新状态。
...
| P3 | viewer 只读账号 | ⏸ 暂缓（2026-08-14 决定先用 root 连接 DBeaver），需要时一条命令可建 | 中 |
| P4 | 服务器 git HTTP/2 抖动 | ⏸ 暂缓（2026-08-17 决定先不动网络配置），推送失败重试即可 | 中 |
...
| # | 任务 | 说明 | 优先级 |
|---|------|------|:--:|
| Q1 | 数据观察期 | ✅ 观察完成（2026-08-24）：进程稳定、整点聚合正常（数据已积累到 8/23）、设备分布与城市解析正常 | 中 |
| Q2 | Jest 单元测试 | ✅ 已完成（2026-08-24）：Jest + ts-jest 配置，19 个用例覆盖 Parser（UA 设备识别/城市解析/内网判断/URL 提取/事件清洗）与 shard（表名生成）。`npm test` 运行 | 中 |
| Q3 | 错误日志巡检 | ✅ 已巡检（2026-08-24）：error.log 自 8-14 降噪后零新错误 | 低 |
